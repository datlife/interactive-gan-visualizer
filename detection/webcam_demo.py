...
def main(_):
	...
	num_frames +=1
        ...
        frame = draw(frame, boxes, classes, scores, camera_fps, detection_fps)
        cv2.imshow('Video', frame)

    elapsed = (time.time() - start)
    print('[INFO] elapsed time (total): {:.2f}'.format(elapsed))
    print('[INFO] approx. FPS: {:.2f}'.format(num_frames / elapsed))
    print('[INFO] approx. detetcion FPS: {:.2f}'.format(detection_frames/ elapsed))
    video_capture.stop()
...
def draw(img, bboxes, classes, scores, fps, detection_fps):
    ...
    for box, category, score in zip(bboxes, classes, scores):
        ...
        draw.text((width - 200, height - 20), "Detection FPS: {:.2f} fps".format(detection_fps), fill=(0, 255, 0), label_size=3, font=font)
    del draw
